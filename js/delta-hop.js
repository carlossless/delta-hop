...
Elm.DeltaHop = Elm.DeltaHop || {};
Elm.DeltaHop.make = function (_elm) {
   "use strict";
   _elm.DeltaHop = _elm.DeltaHop || {};
   if (_elm.DeltaHop.values)
   ...
   function (v) {
      return v === null ? $Maybe.Nothing : $Maybe.Just(typeof v === "object" && "seconds" in v && "timers" in v && "field" in v && "uid" in v && "visibility" in v ? {_: {}
                                                                                                                                                                     ,seconds: typeof v.seconds === "number" ? v.seconds : _E.raise("invalid input, expecting JSNumber but got " + v.seconds)
                                                                                                                                                                     ,timers: _U.isJSArray(v.timers) ? _L.fromArray(v.timers.map(function (v) {
                                                                                                                                                                        return typeof v === "object" && "startTime" in v && "stopTime" in v && "description" in v && "completed" in v && "editing" in v && "running" in v && "id" in v ? {_: {}
                                                                                                                                                                                                                                                                                                                                         ,startTime: typeof v.startTime === "number" ? v.startTime : _E.raise("invalid input, expecting JSNumber but got " + v.startTime)
                                                                                                                                                                                                                                                                                                                                         ,stopTime: typeof v.stopTime === "number" ? v.stopTime : _E.raise("invalid input, expecting JSNumber but got " + v.stopTime)
                                                                                                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                                                                                                         ,running: typeof v.running === "boolean" ? v.running : _E.raise("invalid input, expecting JSBoolean but got " + v.running)
                                                                                                                                                                                                                                                                                                                                         ,id: typeof v.id === "number" ? v.id : _E.raise("invalid input, expecting JSNumber but got " + v.id)} : _E.raise("invalid input, expecting JSObject [\"startTime\",\"stopTime\",\"description\",\"completed\",\"editing\",\"running\",\"id\"] but got " + v);
                                                                                                                                                                     })) : _E.raise("invalid input, expecting JSArray but got " + v.timers)
   ...
   _L.fromArray([A2($Html$Tags.p,
                _L.fromArray([]),
                _L.fromArray([$Html.text("Double-click to edit a timer")]))
                ,A2($Html$Tags.p,
                _L.fromArray([]),
                _L.fromArray([$Html.text("Written by ")
                             ,A2($Html$Tags.a,
                             _L.fromArray([$Html$Attributes.href("https://github.com/mindw0rk")]),
                             _L.fromArray([$Html.text("Karolis Stasaitis")]))]))]));
   ...
   var UpdateTimer = F2(function (a,
   b) {
      return {ctor: "UpdateTimer"
             ,_0: a
             ,_1: b};
   });
   var EditingTimer = F2(function (a,
   b) {
      return {ctor: "EditingTimer"
             ,_0: a
             ,_1: b};
   });
   ...
   var StartStop = F3(function (a,
   b,
   c) {
      return {ctor: "StartStop"
             ,_0: a
             ,_1: b
             ,_2: c};
   });
   ...
   var timerEntry = F2(function (timer,
   s) {
   ...
   var timerList = F3(function (visibility,
   timers,
   s) {
      return function () {
         var cssVisibility = $List.isEmpty(timers) ? "hidden" : "visible";
   ...
   timers) {
      return function () {
         var timersCompleted = $List.length(A2($List.filter,
         function (_) {
            return _.completed;
         },
         timers));
         var timersLeft = $List.length(timers) - timersCompleted;
         var item_ = _U.eq(timersLeft,
         1) ? " item" : " items";
         return A2($Html$Tags.footer,
         _L.fromArray([$Html$Attributes.id("footer")
                      ,$Html$Attributes.hidden($List.isEmpty(timers))]),
         _L.fromArray([A2($Html$Tags.span,
                      _L.fromArray([$Html$Attributes.id("todo-count")]),
                      _L.fromArray([A2($Html$Tags.strong,
                                   _L.fromArray([]),
                                   _L.fromArray([$Html.text($String.show(timersLeft))]))
                      ...
                      _L.fromArray([$Html$Attributes.$class("clear-completed")
                                   ...
                                   ,$Html$Attributes.hidden(_U.eq(timersCompleted,
                                   0))
                                   ,A2($Html$Events.onclick,
                                   actions.handle,
                                   $Basics.always(DeleteComplete))]),
                      _L.fromArray([$Html.text(_L.append("Clear completed (",
                      _L.append($String.show(timersCompleted),
                      ")")))]))]));
      }();
   });
      ...
      return function () {
         ...
         {case "Add":
            ...
            return function () {
                 var update = function (t) {
                    return _U.eq(t.id,
                    action._0) ? !_U.eq(action._1,
                    t.running) ? _U.replace([["running"
                                             ,action._1]
                                            ,["stopTime"
   ...
   $Native$Ports.outgoingSignal(function (v) {
      return {seconds: v.seconds
             ,timers: _L.toArray(v.timers).map(function (v) {
